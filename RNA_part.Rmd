---
title: "Identifying HCC subtypes and the B2 subtype"
author: "Arife Neslihan Kaya"
date: "16/02/2023"
...
knitr::opts_chunk$set(echo = TRUE,warning = FALSE,message = FALSE, tidy = FALSE,fig.pos = "!h")
```

```{r load_libs_and_masterfile, echo=TRUE, message=FALSE, warning=FALSE}
##load libraries
...
NMF was run in R package "NMF" using the Brunet algorithm. Top 3000 most variable genes were used to run NMF. Algorithm number of runs is 200. Below we will analyze transcriptomic subtypes in Asian and European patients in the TCGA cohort.  

```{r Load NMF outputs, echo=TRUE, message=FALSE, warning=FALSE}
# load NMF results
...
We run NMF Brunet algorithm for different ranks from 2 to 6 and 200 runs for each. 
Let's look at cophenetic correlation and silhouette values across ranks to choose optimal number of clusters. 

```{r plor_cophenetic and sillheoutte, echo=TRUE, fig.height=5, fig.width=7.5}
...
For Asians, 4 is the optimal number of cluster. For Caucasians 3 is the optimal number.
Now let's plot consensus matrices.


```{r Asian consensus map, echo=TRUE, fig.height=5, fig.width=13, message=FALSE, warning=FALSE}
...
```{r Caucasian consensus map, echo=TRUE, fig.height=5, fig.width=13, message=FALSE, warning=FALSE}
...
```{r Subtypes, echo=TRUE, warning=FALSE}
...
I have selected 2, 3 or 4 clusters for each cohort. Let's analyze survival differences of these subtypes. Note: We have changed B2 subtype naming to P2 later on. In this documents these two (P2/B2) could have been used to define Asian enriched subtype.

```{r survival analysis asian, echo=TRUE, fig.height=5, fig.width=9, message=FALSE}
...
```{r survival analysis caucasian, echo=TRUE, fig.height=5, fig.width=7, message=FALSE}
...
Using gene signatures from previous HCC subtyping papers, I have calculated a pathway activity score. Patients were assigned to the subtype with highest pathway activity score for some subtypes (Hoshida, Boyault, Chiang, Lee survival) and raw activity scores were used for gene set (lee hepatoblast, Roessler Metastasis). 

Steps (method from Robinson et al. )

* Collect gene sets from MSigDB.
* Convert gene expression of all genes in a gene set to percentiles across patients. 
* Take the mean percentile transformed value across genes for each patient.
* Assign patients to max pathway activity for subtypes (Hoshida, Lee, Chiang, Boyault) , or keep pathway activity score (Roessler metastasis and Lee Hepatoblast subtype (HB))



```{r Activity score Asian, echo=TRUE, fig.height=6, fig.width=10, message=FALSE, warning=FALSE}
...
```{r Activity score Caucasian, echo=TRUE, fig.height=6, fig.width=10, message=FALSE, warning=FALSE}
...
```{r Plot PCA Asian, echo=TRUE, fig.height=3, fig.width=11, warning=FALSE}
...
```
```{r Plot PCA Caucasian, echo=TRUE, fig.height=3, fig.width=8, warning=FALSE}
...
```{r Submap, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Plot_Submap,echo=FALSE, fig.height=3, fig.width=9, message=FALSE, warning=FALSE, paged.print=TRUE}
grid.draw(arrangeGrob(grid.grabExpr(draw(two_st)),
            grid.grabExpr(draw(three_st)),
            grid.grabExpr(draw(four_st)),layout_matrix = rbind(c(1,2,3))))
```

Now I will compare clinical and molecular features between Good (G) and Bad (B) subypes in Asian and 
Caucasian separately. I will also compare G1 and G2 in both cohorts. I can only compare B1 vs B2 in Asian.

## Comparison of features between subtypes
```{r Integration g-b, echo=TRUE, message=FALSE, warning=FALSE}
#Feature list####
...
```{r Integration g1-g2, echo=TRUE, message=FALSE, warning=FALSE}
...
```
```{r B1 vs B2, echo=FALSE, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
asian_difference4_2 <- manipulate_for_feat_comparison(asian4,"ASIAN") %>% 
  filter(subtype%in%c("B1","B2"))

## differences between Bad1 and Bad2 subtypes.
asian_p_values_3 <- sapply(colnames(asian_difference4_2)[!colnames(asian_difference4_2)%in%
                                                           c("subtype")],
                           function(x) 
                             calculate_p_value(asian_difference4_2,"subtype",x)) %>% 
  as.data.frame() %>% tibble::rownames_to_column("feature") %>% 
  rename(asian_p_value=2) %>% 
  mutate(categ=ifelse(feature%in%cl,"Clinical",
                      ifelse(feature%in%ml,"Molecular",
                             ifelse(feature%in%Selected_drivers,"Driver",feature)))) %>% 
  group_by(categ) %>%  
  mutate(asian_p.adj=p.adjust(asian_p_value,"BH")) %>% as.data.frame() %>%  
  select(-categ)


## annotate the subtype with higher value for each feature 
asian_directions_num3 <- (asian_difference4_2 %>% group_by(subtype) %>%
                            summarise_if(is_numeric,function(x) 
                              mean(x,na.rm=T)))[1:2,] %>% 
  apply(.,2,function(x) which.max(x)) %>% unlist() %>% as.data.frame() %>% 
  tibble::rownames_to_column("feature") %>% rename(Asian=2) %>% 
  mutate(Asian=ifelse(Asian==2,"B2","B1"))


asian_directions_char3 <- (asian_difference4_2 %>% group_by(subtype) %>% 
                             summarise_if(is.character,function(x) 
                               sum(x=="1",na.rm = T)))[1:2,] %>% 
  apply(.,2,function(x) which.max(x)) %>% unlist() %>% as.data.frame() %>% 
  tibble::rownames_to_column("feature") %>% rename(Asian=2)%>% 
...
asian_directions3 <- rbind(asian_directions_num3,asian_directions_char3)

assoc_resultB1vsB2 <- asian_p_values_3 %>% 
  mutate(asian_p.adj=ifelse(asian_p.adj < 0.1,"sig",NA)) %>%   
  select(1,3) %>% 
  inner_join(.,asian_directions3) %>% 
  mutate(Asian=ifelse(asian_p.adj=="sig",Asian,NA)) %>% 
  filter(asian_p.adj=="sig") %>% 
  select(-c(asian_p.adj)) %>% 
  melt(.,id.vars=c("feature"))


```

```{r Plot features, echo=TRUE, fig.height=5, fig.width=8, message=FALSE, warning=FALSE}
...
```
When we look at B1 vs B2 in Asian, B2 has higher AFP and purity as well as higher freqeuncyof AXIN1 mutations.
Also B2 is colder. WNT activation because of higher fraction of AXIN1 mutations might be cause of coldness. 

```{r new_comparison_plot, echo=TRUE, fig.height=6, fig.width=5, message=FALSE, warning=FALSE}
...
```{r DEG Comparison B vs G, echo=TRUE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
...
```{r DEG Comparison G1 vs G2, echo=TRUE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
...
```{r DEG Comparison B1 vs B2, echo=TRUE, fig.height=5, fig.width=7, message=FALSE, warning=FALSE}
...
```{r Gene sets, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Asian final plot, echo=TRUE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
...
```{r Caucasian final plot, echo=TRUE, fig.height=10, fig.width=10, message=FALSE, warning=FALSE}
...
```{r B2 features, echo=TRUE, fig.height=5, fig.width=10, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r echo=TRUE, fig.height=15, fig.width=10, message=FALSE, warning=FALSE, paged.print=FALSE}
